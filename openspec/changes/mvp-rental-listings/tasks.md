...
- [ ] 3.13 Drizzle adapters for `ListingRepositoryPort` and `ZoneCataloguePort`, plus the integration test that proves one transaction — **added 2026-08-17, and its absence is the honest reason 3.5 is not yet reachable from a browser.** `save()` deliberately takes the listing and its photos in one call: a listing with no photo row violates a publish rule, so a port offering `saveListing` and `savePhoto` separately would let a caller create exactly that state whenever the second call failed. **Open and unfixed**: if photo processing fails partway through a multi-photo publish, earlier derivatives are already promoted and their originals deleted — orphans in the bucket with no listing row, and nothing sweeps them
