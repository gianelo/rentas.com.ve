...
- [ ] 3.14 Step 2 — photo upload, the one screen where client JS is allowed (SISTEMA.md: "el único lugar donde se permite JS, para comprimir en el dispositivo antes de subir"). Needs the presigned-PUT round trip, then `publishListing`. **Carrying step 1's draft to step 2 is the open design question**, and it must not be a new table: a signed short-lived cookie is the current proposal, since the draft is small once photos are excluded. **In progress**: the presigned-upload gate has shipped — `validateUploadRequest` decides what is worth signing before the R2 adapter or the network are involved, refusing rather than truncating an over-count request, because silently signing the first six of ten publishes a listing missing photos the publisher believes they attached. The `publisherId` segment comes from the session and nothing about the destination comes from the request. Remaining: the client uploader with on-device compression, and the action that runs `processUploadedPhoto` per photo and then `publishListing`
