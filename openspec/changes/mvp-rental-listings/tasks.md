...
- [ ] 3.9 Publish form UI + listing detail/card rendering `publisher_type` visibly — **split into three; the first two have shipped.** (a) `violation-copy.ts`, one Spanish sentence per violation code, a `Record` over the union so a new code stops the build until someone writes it, with set equality asserted in **both** directions against codes the validator actually produced — 17 reachable, 17 written, no dead entries. (b) `PublishForm` + `/publicar`: server-rendered, native `method="post"`, no client JS (SISTEMA.md allows it in step 2 only, for on-device photo compression). **Two fields were added to the design's list after the founder chose to** — `habitaciones` and `metros²` are absent from screen 3's field list yet both columns are NOT NULL, the result row renders `zona · N hab · N m²`, search filters by rooms, and the importer has a Habitaciones column: without them an owner publishing one listing cannot produce a valid row. **The load-bearing assertion is that no publisher type is pre-selected** — the domain refuses a missing type and applies no default precisely so nobody is published as an owner they never claimed to be, and a `defaultChecked` here would restore that default in the one layer the domain cannot see; mutation-checked. Eight fields render through one `Field`, written once because the parts that must not drift are the invisible ones (`aria-invalid`, `aria-describedby`, the 2px error border, the submitted value coming back) and eight hand-written copies is eight chances to drop one and fail silently for exactly the people it exists for. (c) **remaining**: the POST handler that validates and re-renders, step 2 (photos), and rendering `publisher_type` on the detail/card surfaces
- [ ] 3.14 Step 2 — photo upload, the one screen where client JS is allowed (SISTEMA.md: "el único lugar donde se permite JS, para comprimir en el dispositivo antes de subir"). Needs the presigned-PUT round trip, then `publishListing`. **Carrying step 1's draft to step 2 is the open design question**, and it must not be a new table: a signed short-lived cookie is the current proposal, since the draft is small once photos are excluded
