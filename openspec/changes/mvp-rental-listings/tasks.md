...
- [x] 7.1 Schema: `listing.status`/`expires_at`; `listing_reminder` UNIQUE `(listing_id, expires_at)`; `job_run`. **Hecho, PR #99.** Migración 0011. La unicidad lleva TRES columnas `(listing_id, kind, expires_at)` y no dos: con dos, el correo de purga chocaba contra el de vencimiento del mismo ciclo y no salía nunca — justo el que avisa de un borrado irreversible.
- [x] 7.2 RED: listing expires 30 days after publish/last renewal, whichever later. **Hecho, PR #99.**
- [x] 7.3 GREEN: expiry calculation in domain. **Hecho, PR #99.** `expiry.ts`.
- [x] 7.4 RED: unauthenticated `POST /api/jobs/expiry-reminders` returns 401, `reminders_sent=0`. **Hecho, PR #99.** 401 con `reminders_sent: 0`.
- [x] 7.5 GREEN: constant-time Bearer `CRON_SECRET` check on job route. **Hecho, PR #99.** Comparación en tiempo constante, y **falla cerrado**: sin la variable la ruta queda cerrada, no abierta.
- [x] 7.6 RED: double-run of reminder job does not double-send (unique constraint). **Hecho, PR #99.** La garantía es la restricción, no un `if`: `claim()` es un solo `INSERT … ON CONFLICT DO NOTHING … RETURNING` y el puerto no ofrece un `wasSent()`.
- [x] 7.7 GREEN: `SendExpiryRemindersUseCase` — batched 5-day-window select, insert `listing_reminder`, send email, record `job_run` (counts + failures). **Hecho, PR #99.**
- [x] 7.8 RED: replayed renewal token rejected; GET on renewal link never mutates `expires_at`. **Hecho, PR #99.** El `GET` no muta, verificado por mutación.
- [x] 7.9 GREEN: HMAC-signed, listing-scoped, single-use, expiring token; GET renders confirmation, POST renews +30 days and burns token. **Hecho, PR #99.** **Sin tabla de tokens quemados**: el «un solo uso» lo da el propio `UPDATE … WHERE expires_at = ?`, porque renovar mueve la fecha. Sin leer-y-después-escribir no hay ventana para dos clics simultáneos.
- [x] 7.10 RED: expired listing retained (not deleted), excluded from search, still renewable. **Hecho, PR #99.** El vencido se conserva, sale de la búsqueda y sigue renovable.
- [x] 7.11 Resend/React Email renewal template; `vercel.json` cron schedule. **Parcial, PR #99.** El cron del recordatorio está en `vercel.json`. **Falta el proveedor de correo**: `ConsoleLifecycleMailer` avisa que no manda en vez de simular que sí.
...
- [x] 11.9 RED: the expired listing page carries `noindex` and its URL is absent from the sitemap. **Hecho, PR #97.** El sitemap ya excluía las vencidas; ahora la ficha vencida lleva `noindex`.
...
- [x] 11.14 GREEN: schema.org structured data on the listing detail page. **Hecho, PR #97.** `RealEstateListing` → `Accommodation` → `Offer`. **No `Product`**: marcar como producto lo que no lo es dispara una acción manual de Google sobre el dominio entero.
- [x] 11.15 RED: a listing below the minimum content threshold carries `noindex` (thin-content guard for bulk-imported portfolios). **Hecho, PR #97.** Umbral de 300 caracteres, y los delgados salen también del sitemap.
...
- [x] 14.14 The relaxation proposal (F10, F11): evaluate each active filter and offer the single change that adds the most results, with its number. "Ninguna pantalla termina en un vacío sin salida" (criterion 9). **Hecho, PR #101.** Un solo cambio por vez, el que más suma, con su número real.
- [x] 14.15 Zero-results diagnosis (F11): name the filter causing the emptiness, offer up to three concrete exits each with its count. **Never suggest another city** (criterion 8). **Hecho, PR #101.** La causa es verificable: el filtro que soltado solo devuelve más avisos. Si ninguno alcanza, dice «es la combinación de …». **Cero consultas extra**: las salidas salen de la misma consulta que ya contaba las facetas.
...
- [x] 14.20 Works with JavaScript off: the box is a `GET` form; the server translates and redirects to the canonical filtered URL. Live suggestions while typing are an enhancement on top, never the mechanism (F14). **Hecho, PR #98.** `<form method="get">`; el servidor traduce y redirige. Ni un manejador de eventos.
...
- [x] 19.4 A job that deletes the **photos** of listings expired more than 15 days, leaving the listing row and its status untouched. **Hecho, PR #99.** Borra fotos y derivadas y deja la fila del aviso intacta.
- [x] 19.5 **Two emails, not one.** The plan carries one notice *before* expiry, to renew. The purge happens 15 days *after*, so a publisher who ignored the first email loses their photos with no second warning. Day 27: "vence en 3 días". Day 40: "tus fotos se borran el 27 de octubre — renová antes y el aviso vuelve con ellas". **Hecho, PR #99.** Dos correos, con su reserva independiente.
